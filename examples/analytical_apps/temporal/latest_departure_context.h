#ifndef LATEST_DEPARTURE_CONTEXT_H
#define LATEST_DEPARTURE_CONTEXT_H
#include <grape/grape.h>
namespace grape {
template <typename FRAG_T>
class LDContext : public VertexDataContext<FRAG_T, int64_t> {
 public:
  using oid_t = typename FRAG_T::oid_t;
  using vid_t = typename FRAG_T::vid_t;
  explicit LDContext(const FRAG_T& fragment)
  ...
  void Init(AutoParallelMessageManager<FRAG_T>& messages, oid_t src_id,
    ...
    // 定义聚合函数，用于决定什么时候更新信息。
    partial_result.Init(vertices, std::numeric_limits<int64_t>::min(),
